...
def check_server(base_url):
    ...
    assert response.status_code == 201, f"Actual status code is {response.status_code} ({response.request})"
    entity_uuid = response.json()["uuid"]

    entity = session.get(f"/entity/{entity_uuid}")
    assert entity.status_code == 200, f"Actual status code is {response.status_code}"
    assert entity.json()["data"] == data["data"]
