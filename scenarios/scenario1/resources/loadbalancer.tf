...
resource "opentelekomcloud_lb_loadbalancer_v2" "loadbalancer" {
  name          = "${local.workspace_prefix}${var.scenario}_elastic_loadbalancer_http"
...
resource "opentelekomcloud_lb_listener_v2" "listener" {
  name            = "${local.workspace_prefix}${var.scenario}_listener_http"
  protocol        = "TCP"
  protocol_port   = 80
...
resource "opentelekomcloud_lb_pool_v2" "pool" {
  name        = "${local.workspace_prefix}${var.scenario}_pool_http"
  protocol    = "TCP"
...
resource "opentelekomcloud_lb_monitor_v2" "monitor" {
  name        = "${local.workspace_prefix}${var.scenario}_monitor_http"
  pool_id     = opentelekomcloud_lb_pool_v2.pool.id
  type        = "TCP"
  delay       = 1
  timeout     = 1
